...
namespace Bookstore.Services.Services
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<IEnumerable<Employee>> GetAllEmployeesSortedBySurname(bool isDescending)
        {
            if (isDescending)
            {
                return (await _employeeRepository.GetAll()).OrderByDescending(x => x.Surname);
            }
            return (await _employeeRepository.GetAll()).OrderBy(x => x.Surname);
        }

        public async Task<IEnumerable<Employee>> GetEmployeeesByStatus(EmployeeStatus status)
        {
            return (await _employeeRepository.GetAll()).Where(x => x.EmployeeStatus == status);
        }

        public async Task<IEnumerable<Employee>> GetAllEmployeesByPosition(EmployeePosition position)
        {
            return (await _employeeRepository.GetAll()).Where(x => x.EmployeePosition == position);
        }
